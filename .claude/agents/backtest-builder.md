...
5. Reference exemplars to copy patterns from, NOT re-derive: `signal_event.py` (event-merge + maker fill gate `book_ask<=bid` + depth cap), `bracket_hit_backtest.py::noon()` (canonical slug→noon-ET), `phase_wh_maker.py` (strict through-fill `p<b`, calib_sigma, cap15), `trade_sim.py` (correct scoring use of `winner`), `odds_vs_market.py` / `calibration_test.py` (model-vs-market Brier).
