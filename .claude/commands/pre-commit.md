Staged files: !`git diff --cached --name-only`
Staged diff: !`git diff --cached`

Run in sequence. STOP and report if critical issues found:
1. @qa-code-bug-hunter — bugs, edge cases, security, performance, trading-specific risks
2. @strategy-reviewer — if any files in api/modules/ or api/services/risk* changed
3. @risk-auditor — if any files touching orders, execution, or positions changed
4. @verify-bot — if trading logic changed, run full verification

Only proceed to commit if all checks pass.
Report a summary of findings before committing.

NOTE: @qa-code-quality and @qa-architecture-quality are NOT in this chain by design.
They are heavier sweeps (orphan code, duplication, hotfix archaeology, module boundaries,
abstraction leaks) meant for periodic audits, NOT every commit. Invoke them on demand:
  - Weekly or before major refactors → @qa-code-quality
  - Monthly or before adding a new module → @qa-architecture-quality
