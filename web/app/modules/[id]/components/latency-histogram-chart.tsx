...
import { chartTooltip, fmtDuration } from "@/lib/utils"
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            This module's orders only — time from post detection to fill.{" "}
            <span className="text-muted-foreground/70">Median is the realistic number; avg is skewed by parked limit orders.</span>
          </p>
        </div>
        {data && data.sample_count > 0 && (
          <div className="flex gap-3 text-xs text-muted-foreground">
            <span>Avg: <span className="text-foreground font-medium">{fmtDuration(data.avg_latency_s)}</span></span>
            <span>Median: <span className="text-foreground font-medium">{fmtDuration(data.median_latency_s)}</span></span>
