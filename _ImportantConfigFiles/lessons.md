...
### 2026-09-04 — The fix was merged and never deployed; the alert that would have said so was swallowed
**What happened**: Sir asked for a status update and got: zero fills for 22 days (last fill 2026-08-13 10:46 UTC), 8,448 SELL orders resting up to 56 days, S2 Basket-Hold and Copytrader emitting no signals at all. Every one of those was supposedly fixed by the 2026-09-01 nightly-QA work. The repo was correct: a standalone trace of S2's `live_auction_snapshot` + decision layer against live Gamma/xTracker produced two valid signals on the spot. The Dublin box was running `0f23f10` on branch `feat/newbot-step1-skeleton`, dated 2026-07-26, **100 commits behind master**. The year-regex fix, the SELL-sweep fix and the four-silent-modules fix had all been merged and none had ever shipped. Redeploying master took the very next cycle from `signals=22 approved=0 paper_fills=0` to `signals=28 approved=12 paper_fills=4`, then 7 fills on the cycle after. Separately, `scripts/daily_verify.py` had correctly detected and logged `fill path broken` EVERY DAY since 2026-08-26 — its Telegram send sat inside a bare `except: pass`, and `notify()` only falls back to Slack when Telegram fails, so the one message that mattered was both unverified and buried under the ~40 routine `tier_*`/`dir_*` alerter messages a day. Two real code bugs surfaced during the dig: the duplicate-resting-BUY guard keyed on `(module, market, bracket)` without `token_id`, so a complement-pair arb always had its second leg rejected and sat one-legged (which is what `_orphan_exits` was written to mop up); and the ghost writer was finally identified from Supabase edge logs as IP `152.55.180.100` (Railway, Ashburn) using the ORIGINAL `default` secret key `sb_secret_6ZhCH…`, which the 2026-07-29 rotation created a replacement for but never deleted.
**Root cause**: I diagnosed live behaviour from repo code. Every module, gate and data path read correctly, and I spent the investigation looking for a logic bug that did not exist. Nothing in the database distinguishes "the fix is not deployed" from "the fix does not work" — only the box's SHA does, and I checked it last instead of first. The detector that existed to prevent exactly this had been firing into a channel nobody could hear, with no record of whether it was ever delivered.
**Rule**: (a) **The first command of any live investigation is `ssh polybot@34.245.42.217 "cd ~/bot && git log --oneline -1 && git rev-parse --abbrev-ref HEAD"`** — state the deployed SHA out loud and diff it against `origin/master` before analysing anything. A merged PR is not a shipped fix. (b) The signature of deploy drift: repo reads correctly, a standalone trace of the same function returns the right answer, production behaves as if the fix were absent — suspect the deploy before the logic. (c) **Never wrap a notification in a bare `except: pass`.** Capture the boolean, write `alert delivered=<bool>` to `logs`, send critical alerts to Telegram AND Slack (never else-fallback), and carry a consecutive-broken-days counter so day 22 does not read like day 1. A detector firing into the void is worse than no detector because it manufactures the illusion of coverage. (d) A dedupe key must include every field that makes two orders genuinely different — `token_id` is load-bearing whenever one bracket label covers both YES and NO. (e) When a bulk backlog (8,448 rows) blocks a gate, purge it with one SQL `UPDATE` rather than waiting for a per-cycle sweep that batches 50 at a time. (f) Identify a rogue Supabase writer from `edge_logs`: group by `request.sb.apikey.apikey.prefix`, `request.headers.cf_connecting_ip` and `request.cf.asOrganization` — it named the key and the host in one query after months of guessing. Rotating a key does nothing until the OLD key is deleted.
