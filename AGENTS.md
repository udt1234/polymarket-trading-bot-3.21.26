...
2. **`~/.Codex/projects/C--Users-darwi-OneDrive-Desktop-Codex-Personal-PolyMarket-Bot/memory/lesson_<slug>.md`** — auto-loaded memory file with the RULE (concise, actionable). Then add a one-line pointer to `memory/MEMORY.md` under the "Hard-won lessons" section.
